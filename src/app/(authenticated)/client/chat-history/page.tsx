...
import { ChatSessionCard, ChatSession } from "@/components/client/ChatSessionCard";
...
export default function ChatHistoryPage() {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      if (data.error) {
        const errorMessage = data.error || "An error occurred while fetching chat sessions";
        throw new Error(errorMessage);
      }
            ...
            try {
              ...
              if (messagesResponse.ok) {
                ...
                if (!messagesContentType?.includes("application/json")) {
                  console.warn(`Invalid content type for messages response for session ${sessionItem.id}`);
                  return { ...sessionItem, messageCount: 0, lastMessage: "", chatType: "wizard" as const };
                }
                ...
                if (!messagesData || typeof messagesData !== "object") {
                  console.warn(`Invalid messages response structure for session ${sessionItem.id}`);
                  return { ...sessionItem, messageCount: 0, lastMessage: "", chatType: "wizard" as const };
                }

                // Handle error in messages response
                if (messagesData.error) {
                  console.warn(`Error fetching messages for session ${sessionItem.id}:`, messagesData.error);
                  return { ...sessionItem, messageCount: 0, lastMessage: "", chatType: "wizard" as const };
                }
                ...
                const messages = Array.isArray(messagesData.messages) 
                  ...
                  : (Array.isArray(messagesData.data?.messages) ? messagesData.data.messages : []);
                ...
                ) ? "grand-wizard" as const : "wizard" as const;
                ...
                console.warn(`Failed to fetch messages for session ${sessionItem.id}: ${messagesResponse.status}`);
                return { ...sessionItem, messageCount: 0, lastMessage: "", chatType: "wizard" as const };
              }
            } catch (msgError) {
              // Log the error but don't fail the entire operation
              console.error(`Error fetching messages for session ${sessionItem.id}:`, msgError);
              return { ...sessionItem, messageCount: 0, lastMessage: "", chatType: "wizard" as const };
            }
          })
        );
      ...
      } else if (data.success && (!data.sessions || !Array.isArray(data.sessions))) {
  ...
  const handleDelete = async (sessionId: string) => {
    ...
    try {
      ...
      // Handle error responses
      if (data.error) {
        const errorMessage = data.error || "An error occurred while deleting the session";
        throw new Error(errorMessage);
      }

...
export async function GET(request: NextRequest) {
  try {
    ...
    redirectResponse.cookies.set('microsoft_access_token', tokenData.access_token, {
    ...
    if (tokenData.refresh_token) {
      redirectResponse.cookies.set('microsoft_refresh_token', tokenData.refresh_token, {
        httpOnly: false, // Allow client-side access
        secure: process.env.NODE_ENV === 'production',
        sameSite: 'lax',
    ...
    redirectResponse.cookies.set('microsoft_token_expiry', (Date.now() + (tokenData.expires_in * 1000)).toString(), {
      httpOnly: false,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'lax',
      maxAge: tokenData.expires_in

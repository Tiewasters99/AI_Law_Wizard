...
const clientNavigation: Record<string, NavigationSection> = {
  findLegalHelp: {
    ...
    icon: Search,
    items: [
      { label: 'Find Attorney', href: '/directory', icon: Search, badge: null },
      { label: 'My Requests', href: '/inbox', icon: FileCheck, badge: 'pending' },
    ]
  },
  aiTools: {
    label: 'AI Tools',
    icon: Bot,
    items: [
      { label: 'Legal Assistant', href: '/wizard', icon: Bot },
      { label: 'Advanced Assistant', href: '/grand-wizard', icon: Crown },
      { label: 'Chat History', href: '/legal-chat', icon: History },
    ]
  },
  mySpace: {
    label: 'My Space',
    icon: Folder,
    items: [
      { label: 'Messages', href: '/inbox', icon: MessageSquare, badge: 'unread' },
      { label: 'My Documents', href: '/integrations', icon: Files },
    ]
  },
  resources: {
    label: 'Resources',
  ...
  account: {
    ...
    items: [
      ...
      { label: 'My Credits', href: '/tokens', icon: Coins },
    ]
  }
}

export function ClientSidebar({ isCollapsed, onToggle, unreadCount = 0, pendingRequestsCount = 0 }: ClientSidebarProps) {
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  ...
                  const showBadge = 
                    ...
                    (item.badge === 'pending' && pendingRequestsCount > 0)
                  const badgeCount = item.badge === 'unread' ? unreadCount : pendingRequestsCount

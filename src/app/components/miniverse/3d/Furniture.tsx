...
export const Furniture: React.FC<FurnitureProps> = ({ config, onItemClick }) => {
  ...
  return (
    <group>
      ...
      {config.objects.table.visible && (
        <mesh position={[config.objects.table.position.x, config.objects.table.position.y, config.objects.table.position.z]}>
          <boxGeometry args={[10, 0.2, 5]} />
          <meshStandardMaterial color={config.objects.table.color || '#1f2937'} roughness={0.2} metalness={0.3} />
        </mesh>
      )}
      ...
      {config.objects.bookshelf.visible && (
        <mesh position={[config.objects.bookshelf.position.x, config.objects.bookshelf.position.y, config.objects.bookshelf.position.z]}>
          <boxGeometry args={[0.4, 6, 8]} />
          <meshStandardMaterial color={config.objects.bookshelf.color || '#4a2c1a'} />
        </mesh>
      )}
      ...
      {config.objects.reception.visible && (
        <mesh position={[config.objects.reception.position.x, config.objects.reception.position.y, config.objects.reception.position.z]}>
          <boxGeometry args={[3, 1, 1.5]} />
          <meshStandardMaterial color={config.objects.reception.color || '#3d2817'} />
        </mesh>
      )}

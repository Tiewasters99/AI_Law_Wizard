      ...
      <div className="p-4 h-[calc(100vh-140px)] overflow-y-auto">
        {activeTab === 'scene' && (
          <div className="space-y-6">
            <div>
              <h4 className="text-sm font-semibold text-white mb-4">Room Colors</h4>
        ...
        {activeTab === 'objects' && (
          <div className="space-y-6">
            <div>
              <h4 className="text-sm font-semibold text-white mb-4">Room Objects</h4>
              
              {/* Chairs */}
              <div className="mb-6">
                <h5 className="text-xs font-medium text-gray-300 mb-3">Chairs</h5>
                <div className="space-y-3">
                  {config.objects.chairs.map((chair, index) => (
                    <div key={chair.id} className="bg-slate-700/30 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs text-gray-300">Chair {index + 1}</span>
                        <label className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            checked={chair.visible}
                            onChange={(e) => updateObject('chairs', chair.id, { visible: e.target.checked })}
                      ...
                      <PositionControls
                        ...
                        onChange={(position) => updateObject('chairs', chair.id, { position })}
                      />
                    </div>
                  ))}
                </div>
              ...
              {/* Table */}
              <div className="mb-6">
                <h5 className="text-xs font-medium text-gray-300 mb-3">Conference Table</h5>
                <div className="bg-slate-700/30 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <label className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) => updateObject('table', config.objects.table.id, { visible: e.target.checked })}
                        className="w-3 h-3 rounded border-slate-400/40 bg-slate-600"
                      />
                      <span className="text-xs text-gray-400">Visible</span>
                    </label>
                  ...
                  <PositionControls
                    ...
                    onChange={(position) => updateObject('table', config.objects.table.id, { position })}
                  />
                  <ColorPicker
                    label="Color"
                    value={config.objects.table.color || '#1f2937'}
                    onChange={(color) => updateObject('table', config.objects.table.id, { color })}
                    className="mt-3"
                  />
                </div>
              </div>
              ...
              {/* Lamps */}
              <div className="mb-6">
                <h5 className="text-xs font-medium text-gray-300 mb-3">Lamps</h5>
                <div className="space-y-3">
                  {config.objects.lamps.map((lamp, index) => (
                    <div key={lamp.id} className="bg-slate-700/30 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs text-gray-300">Lamp {index + 1}</span>
                        <label className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            checked={lamp.visible}
                            onChange={(e) => updateObject('lamps', lamp.id, { visible: e.target.checked })}
                            className="w-3 h-3 rounded border-slate-400/40 bg-slate-600"
                          />
                          <span className="text-xs text-gray-400">Visible</span>
                        </label>
                      ...
                      <PositionControls
                        ...
                        onChange={(position) => updateObject('lamps', lamp.id, { position })}
                      />
                    </div>
                  ))}
                </div>
              ...
              {/* Plants */}
              <div className="mb-6">
                <h5 className="text-xs font-medium text-gray-300 mb-3">Plants</h5>
                <div className="space-y-3">
                  {config.objects.plants.map((plant, index) => (
                    <div key={plant.id} className="bg-slate-700/30 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs text-gray-300">Plant {index + 1}</span>
                        <label className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            checked={plant.visible}
                            onChange={(e) => updateObject('plants', plant.id, { visible: e.target.checked })}
                            className="w-3 h-3 rounded border-slate-400/40 bg-slate-600"
                          />
                          <span className="text-xs text-gray-400">Visible</span>
                        </label>
                      ...
                      <PositionControls
                        ...
                        onChange={(position) => updateObject('plants', plant.id, { position })}
                      />
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'panels' && (
          <div className="space-y-6">
            <div>
              <h4 className="text-sm font-semibold text-white mb-4">Interactive Panels</h4>
              <div className="space-y-4">
                {Object.entries(config.panels).map(([panelId, panel]) => (
                  <div key={panelId} className="bg-slate-700/30 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <label className="flex items-center space-x-2">
                        <input
                          ...
                          onChange={(e) => updatePanel(panelId, { visible: e.target.checked })}
                          className="w-3 h-3 rounded border-slate-400/40 bg-slate-600"
                        />
                        <span className="text-xs text-gray-400">Visible</span>
                      </label>
                    ...
                    <div className="mt-3 space-y-2">
                      <label className="text-xs text-gray-400">Size (W × H × D)</label>
                      <div className="grid grid-cols-3 gap-2">
                        <input
                          type="number"
                          value={panel.size[0]}
                          onChange={(e) => updatePanel(panelId, { 
                            size: [parseFloat(e.target.value), panel.size[1], panel.size[2]] 
                          })}
                        ...
                        <input
                          ...
                          value={panel.size[1]}
                          onChange={(e) => updatePanel(panelId, { 
                            size: [panel.size[0], parseFloat(e.target.value), panel.size[2]] 
                          })}
                          className="w-full px-2 py-1 text-xs bg-slate-600 border border-slate-400/40 rounded text-white"
                          step="0.1"
                        />
                        <input
                          ...
                          value={panel.size[2]}
                          onChange={(e) => updatePanel(panelId, { 
                            size: [panel.size[0], panel.size[1], parseFloat(e.target.value)] 
                          })}
                          className="w-full px-2 py-1 text-xs bg-slate-600 border border-slate-400/40 rounded text-white"
                          step="0.1"
                        />
                    ...
                    <div className="mt-3">
                      <label className="text-xs text-gray-400">Content URL (optional)</label>
                      <input
                        type="url"
                        value={panel.contentUrl || ''}
                        onChange={(e) => updatePanel(panelId, { contentUrl: e.target.value })}
        ...
        {activeTab === 'export' && (
          <div className="space-y-6">
            <div>
              ...
              <div className="space-y-4">
                <div className="bg-slate-700/30 rounded-lg p-3">
                  <h5 className="text-xs font-medium text-gray-300 mb-2">Export Configuration</h5>
                  <p className="text-xs text-gray-400 mb-3">Download your current configuration as a JSON file</p>
                  <button
                ...
                <div className="bg-slate-700/30 rounded-lg p-3">
                  <h5 className="text-xs font-medium text-gray-300 mb-2">Import Configuration</h5>
                  <p className="text-xs text-gray-400 mb-3">Upload a previously saved configuration file</p>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept=".json"
                ...
                <div className="bg-slate-700/30 rounded-lg p-3">
                  <h5 className="text-xs font-medium text-gray-300 mb-2">Reset Configuration</h5>
                  <p className="text-xs text-gray-400 mb-3">Reset to default configuration</p>
                  <button
                    onClick={() => {

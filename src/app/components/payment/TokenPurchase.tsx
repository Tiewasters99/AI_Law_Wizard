...
import { getStripe, fetchTokenPackages, fetchWallet, TokenPackage, Wallet, formatPrice, createPaymentIntent } from '@/app/lib/stripe';
...
export const TokenPurchase = ({ onSuccess, showWallet = true }: TokenPurchaseProps) => {
      ...
      <div className="space-y-6">
        ...
        <Card className="border-blue-200 bg-blue-50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-semibold text-blue-900">{selectedPackage.name}</h4>
                <p className="text-blue-700">{selectedPackage.tokens} tokens</p>
                {selectedPackage.description && (
                  <p className="text-sm text-blue-600 mt-1">{selectedPackage.description}</p>
                )}
              </div>
              <div className="text-right">
                ...
                <div className="text-sm text-blue-600">
                  {formatPrice(selectedPackage.priceInCents / selectedPackage.tokens)} per token
                </div>
              </div>
            </div>
          </CardContent>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="text-center">
          ...
          <p className="text-gray-600">Choose a token package to continue with AI analysis</p>
        </div>
          ...
          {packages.map((pkg, index) => (
            <Card
              ...
              className={`relative cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-105 ${
                index === 0 ? 'border-2 border-blue-500 shadow-lg' : 'border border-gray-200'
              }`}
            ...
            >
              ...
              <CardHeader className="text-center pb-4">
                <div className={`w-16 h-16 mx-auto rounded-2xl flex items-center justify-center mb-3 ${
                  ...
                  <Zap className={`w-8 h-8 ${index === 0 ? 'text-blue-600' : 'text-gray-600'}`} />
                </div>
              ...
              <CardContent className="text-center">
                ...
                <div className="space-y-2 mb-6">
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <span className="text-sm">{pkg.tokens} AI Analysis Tokens</span>
                  </div>
                  <div className="flex items-center justify-center space-x-2">
                    <Check className="w-4 h-4 text-green-600" />

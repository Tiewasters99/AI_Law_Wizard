...
import type { PacerCredentials, PacerAuthResponse, PacerSession } from '@/types/pacer'
...
export function usePacerAuth(): UsePacerAuthReturn {
  ...
  const [userInfo, setUserInfo] = useState<PacerAuthResponse['userInfo'] | null>(null)
   ...
   * Login to PACER
   */
  const login = useCallback(async (credentials: PacerCredentials): Promise<boolean> => {
    console.log('[usePacerAuth] Login attempt with credentials:', { username: credentials.username })
    
    setLoading(true)
    setError(null)

    try {
      console.log('[usePacerAuth] Calling /api/pacer/auth...')
      ...
      console.log('[usePacerAuth] Auth API response status:', response.status)
      ...
      console.log('[usePacerAuth] Session token:', data.sessionToken?.substring(0, 20) + '...')
      ...
      console.log('[usePacerAuth] State updated - isAuthenticated: true, sessionToken set')

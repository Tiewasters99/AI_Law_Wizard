...
interface MiniverseState {
  ...
  updateObject: (type: keyof MiniverseConfig['objects'], objectId: string, updates: Partial<ObjectConfig>) => void
...
const defaultConfig: MiniverseConfig = {
  ...
  objects: {
    chairs: [
      { id: 'chair-1', position: { x: -1, y: 0, z: 7.5 }, rotation: Math.PI, color: '#1f2937', visible: true },
      { id: 'chair-2', position: { x: 1, y: 0, z: 7.5 }, rotation: Math.PI, color: '#1f2937', visible: true },
      { id: 'chair-3', position: { x: -6, y: 0, z: 4 }, rotation: Math.PI / 2, color: '#1f2937', visible: true },
      { id: 'chair-4', position: { x: 6, y: 0, z: 4 }, rotation: -Math.PI / 2, color: '#1f2937', visible: true },
      { id: 'chair-5', position: { x: -1, y: 0, z: 0.5 }, rotation: 0, color: '#1f2937', visible: true },
      { id: 'chair-6', position: { x: 1, y: 0, z: 0.5 }, rotation: 0, color: '#1f2937', visible: true }
    ],
    lamps: [
    ...
    table: { id: 'table-1', position: { x: 0, y: 0.8, z: 4 }, color: '#1f2937', visible: true },
    bookshelf: { id: 'bookshelf-1', position: { x: -24.5, y: 3, z: -8 }, color: '#4a2c1a', visible: true },
    reception: { id: 'reception-1', position: { x: -24, y: 0.5, z: 0 }, color: '#3d2817', visible: true }
  },
  panels: {
    'firm-videos': {
      id: 'firm-videos',
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        updateWallColor: (color) => set((state) => ({
          config: {
            ...state.config,
        ...
        updateFloorColor: (color) => set((state) => ({
          config: {
            ...state.config,
        ...
        updateObject: (type, objectId, updates) => set((state) => {
          if (type === 'table' || type === 'bookshelf' || type === 'reception') {
            // Single objects
            return {
              config: {
                ...state.config,
        ...
        importConfig: (jsonString) => {
          try {
            ...
            // Basic validation
            if (!importedConfig.walls || !importedConfig.floor || !importedConfig.objects || !importedConfig.panels) {
              return false
            }

...
export const WallPanels: React.FC<WallPanelsProps> = ({
  config,
  onPanelClick,
}) => {
  ...
  // Text textures
  const rightWallBrandTexture = createTextTexture(
    "QUAINTON LAW MINIVERSE™",
    80
  );
  const backWallFarBrandTexture = createTextTexture(
    "QUAINTON LAW MINIVERSE™",
    80,
    "#ffffff"
  );
      ...
      {Object.entries(config.panels).map(([panelId, panel]) =>
        panel.visible ? (
          <group key={panelId}>
            ...
            >
              <planeGeometry
                args={[panel.size[0] * 0.8, panel.size[1] * 0.3]}
              />
              <meshBasicMaterial
                map={createTextTexture(panel.label, 60, "#ffffff")}
                transparent
              />
            </mesh>
          </group>
      ...
      {PROFILE_POSITIONS.map((position, index) => (
        <group key={`profile-${index}`}>
          ...
          <mesh position={[position, 2.5, 24.8]} rotation={[0, Math.PI, 0]}>
            ...
            <meshBasicMaterial
              map={createTextTexture("PROFILES", 40, "#ffffff")}
              transparent
            />
          </mesh>
        </group>

...
export const FloorPlanView: React.FC = () => {
  ...
  const { config, updateObject, updatePanel, setViewMode } =
    useMiniverseStore();
  ...
  const drawFloorPlan = useCallback(() => {
    ...
    ctx.fillRect(
      roomPos.x - cornerSize / 2,
      roomPos.y - cornerSize / 2,
      cornerSize,
      cornerSize
    );
    ctx.fillRect(
      roomPos.x + roomSizeCanvas - cornerSize / 2,
      roomPos.y - cornerSize / 2,
      cornerSize,
      cornerSize
    );
    ctx.fillRect(
      roomPos.x - cornerSize / 2,
      roomPos.y + roomSizeCanvas - cornerSize / 2,
      cornerSize,
      cornerSize
    );
    ctx.fillRect(
    ...
    // Draw wall panels
    items
      .filter(item => item.category === "wall")
      .forEach(item => {
        if (!item.visible) return;
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (clickedItem) {
        ...
        setDragOffset({
          x:
            x - worldToCanvas(clickedItem.position.x, clickedItem.position.z).x,
          y:
            y - worldToCanvas(clickedItem.position.x, clickedItem.position.z).y,
        });
        setIsDragging(true);
      }
    },
    ...
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (draggedItem.type === "object") {
        ...
        for (const objectType of objectTypes) {
          const objects =
            config.objects[objectType as keyof typeof config.objects];
          if (Array.isArray(objects)) {
            const found = objects.find(obj => obj.id === draggedItem.id);
            if (found) {
              updateObject(objectType as any, draggedItem.id, {
      ...
      <div className="absolute bottom-4 left-4 z-10">
        <div className="backdrop-blur-xl bg-slate-800/60 border border-slate-400/40 rounded-lg p-3">
          <div className="text-xs text-gray-300">
            ...
            <div>
              • Drag wall panels (rectangles) to reposition on walls only
            </div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-500/40">
              <div>
                • Press{" "}
                <kbd className="px-1 py-0.5 bg-slate-700 rounded text-[10px]">
                  Esc
                </kbd>{" "}
                or{" "}
                <kbd className="px-1 py-0.5 bg-slate-700 rounded text-[10px]">
          ...
          <div className="bg-white rounded-2xl p-6 w-11/12 max-w-md">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                Your changes will be applied and you&apos;ll switch to 3D
                viewing mode to see your office space in 3D.
              </p>
            </div>

...
export const MusicBrowser: React.FC<MusicBrowserProps> = ({
  ...
  const handleAddTrack = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isLocalFile =
        newTrackUrl.startsWith("/music/") && newTrackUrl.endsWith(".mp3");

      if (!isYouTubeUrl && !isLocalFile) {
        throw new Error(
          "Please enter a valid YouTube URL (e.g., https://www.youtube.com/watch?v=VIDEO_ID) or local file path (e.g., /music/file.mp3)"
        );
      }
      ...
      if (isYouTubeUrl) {
        ...
        if (newTrackUrl.includes("youtube.com/embed/")) {
          ...
          const videoIdMatch = newTrackUrl.match(
            /youtube\.com\/embed\/([^&\n?#]+)/
          );
          if (videoIdMatch && videoIdMatch[1]) {
            watchUrl = `https://www.youtube.com/watch?v=${videoIdMatch[1]}`;
            console.log(
              "Converted embed URL to watch URL:",
              newTrackUrl,
              "->",
              watchUrl
            );
      ...
      <div className="bg-gradient-to-br from-slate-800/90 via-slate-900/90 to-slate-950/90 backdrop-blur-xl border border-slate-400/40 rounded-2xl shadow-2xl max-w-6xl w-full max-h-[85vh] overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    💡 Tip: Use watch URLs (youtube.com/watch?v=) instead of
                    embed URLs for better compatibility
                  </div>
                </div>
          ...
          {filteredTracks.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mb-4">
                Add YouTube URLs or local MP3 files to get started
              </p>
              <button
        ...
        <div className="p-4 border-t border-slate-600/40 bg-slate-800/30">
          <div className="flex items-center justify-between text-xs text-gray-400">
            <span>
              🎵 Add YouTube URLs or local MP3 files to your Miniverse library
            </span>

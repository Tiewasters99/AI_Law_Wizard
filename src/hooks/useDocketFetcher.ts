...
export function useDocketFetcher(): UseDocketFetcherReturn {
  const [docketData, setDocketData] = useState<DocketReportResponse | null>(
    null
  );
  const [feeEstimate, setFeeEstimate] = useState<DocketFeeEstimate | null>(
    null
  );
  ...
  const estimateFee = useCallback(
    async (sessionToken: string, caseNumber: string, court: string) => {
      try {
        setLoading(true);
        setError(null);
        setFeeEstimate(null);
        ...
        const response = await fetch(
          "/api/attorney/pacer/docket/estimate-fee",
          {
            method: "POST",
            headers: {
  ...
  const fetchDocket = useCallback(
    async (sessionToken: string, caseNumber: string, court: string) => {
      try {
        ...
        const response = await fetch("/api/attorney/pacer/docket", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },

...
export function usePacerSearch(): UsePacerSearchReturn {
  ...
  const searchCases = useCallback(
    async (query: PacerSearchQuery, sessionToken: string): Promise<void> => {
      console.log("[usePacerSearch] searchCases called with:", {
        query,
      ...
      try {
        console.log(
          "[usePacerSearch] Making API call to /api/attorney/pacer/search"
        );

        const response = await fetch("/api/attorney/pacer/search", {
          method: "POST",
          headers: {
        ...
        if (data.cases.length > 0) {
          console.log(
            `Found ${data.totalCount} case${data.totalCount !== 1 ? "s" : ""}`
          );
        } else {
          console.log("No cases found matching your search criteria");
        }
      } catch (err) {
        const errorMessage =
  ...
  const loadMore = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const data: PacerSearchResults & { success: boolean; message?: string } =
        await response.json();
      ...
      console.log(`Loaded page ${data.page} of ${totalPages}`);
    } catch (err) {
      const errorMessage =
